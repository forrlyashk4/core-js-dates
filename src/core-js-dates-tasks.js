...
function getDayName(date) {
  switch (new Date(date).getDay()) {
    ...
    case 6:
      return 'Saturday';
    default:
      return new Error('Not a day / Not a date');
  }
}
...
function getWeekNumberByDate(date) {
  const startOfTheYear = new Date(date.getFullYear(), 0);

  let counter = 1;
  while (startOfTheYear.valueOf() !== date.valueOf()) {
    if (startOfTheYear.getDay() === 0) counter += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate.valueOf() <= endDate.valueOf()) {
    if (workDayCounter !== countWorkDays) {
      ...
      workDaySchedule.push(`${day}-${month}-${year}`);
      ...
      if (vacationCounter === countOffDays) workDayCounter = 0;
    }
